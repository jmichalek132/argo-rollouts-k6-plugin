...
- [ ] **GC-01**: Metric plugin implements `GarbageCollect(ar *v1alpha1.AnalysisRun, metrics []*v1alpha1.Measurement) error` — when called by Argo Rollouts after terminal analysis, deletes every k6-operator TestRun CR the plugin created for that AnalysisRun (CR name + namespace are recoverable from `metric.Metadata["runId"]` + AR namespace). Cascade deletes runner pods via Kubernetes owner references. No-op when `provider: grafana-cloud` (nothing to clean up).
